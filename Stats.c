...
{
   //Check if any customers exist to avoid dividing by 0
   int cust = stats->customersServed;
   
   printf(" - Customers Served:   %d\n",stats->customersServed);
   printf(" - Service Time:       %d\n",stats->totalServiceTime);
   printf(" - Avg Service Time:   %.2lf\n",cust == 0 ? 0 : (double) stats->totalServiceTime / cust);
   printf(" - Wait Time:          %d\n",stats->totalWaitTime);
   printf(" - Avg Wait Time:      %.2lf\n",cust == 0 ? 0 : (double) stats->totalWaitTime / cust);
   printf(" - Largest Queue Size: %d\n",stats->largestQueueSize);
